...
To investigate whether Double Stuf Oreos have twice the créme filling mass as regular Oreos, I need a well designed study. In this situation, I am going to design our study to limit my Type I Error Rate to no more than 3% ($\epsilon_{I}=0.03$). Further, I'll set the Type II Error Rate to 20% ($\epsilon_{II}=0.2$); alternatively, I'll have the study have a power of 80%. Based on the prior studies, there is some evidence of large effect size in moving away from a ratio of 2 in filling masses. From these parameters, we would need 60 total cookies, evenly split between the two types for a balanced study. Given the provided data, we will have an imbalanced design with 36 regular Oreos and 30 Double Stuf Oreos. 

I will make use of data previously collected for this purpose. Dr. Hatfield worked with an introductory statistics class to investigate whether Double Stuf Oreos have twice the créme filling as regular Oreos. They used a proper process to select a grocery store at random in Tempe, AZ and then to select 9 packages of regular and 9 packages of Double Stuf Oreos from the shelves. They then split these 9 packages into 9 pairs (one of each type of cookie) to measure the créme filling masses (in grams) for each cookie.
...
#|   - "Fixed Data"
#| layout-ncol: 2
#| fig-alt: "The left box plots show the original data for the two types of Oreos while the right box plots show the corrected data for the mislabelled observations."
...
@fig-shadowgram provides a glimpse at modal clumps of cookies based on their filling masses. There appear to be two modal clumps (where the shadows are heaviest) around 3-3.5 grams and then around 7 grams. These modal clumps appear to line up with the midpoints of the regular and Double Stuf Oreos, respectively, from @fig-boxplots1-2

I also wanted to look at a histogram to get a better sense of the créme filling masses. @fig-histograms provides us with side-by-side histograms for the two types of cookies. We can see a match between the centers of the modal clumps in the shadowgram (@fig-shadowgram) and the location of the highest bars in @fig-histograms. Further, we can get a sense of the spread of the filling masses. The Double Stuf cookies appear to go from a little under 6 grams to over 7.5 grams; the Regular cookies vary from a bit under 2.5 grams to just over 4 grams.
...
#| fig-height: 2
#| fig-pos: "H"
#| fig-alt: "The left histogram shows the Double Stuf Oreos tending to have a little more than 7 grams of filling. The right histogram shows regular Oreos as having around 3.25 grams of filling."
...
In addition to using data visualizations, I've also used used several descriptive statistics on filling mass by the type of cookie as displayed in @tbl-descStatsTable. We can quickly see that there are `r groupStats["Regular", "n"]` regular Oreos and `r groupStats["Double Stuf", "n"]` Double Stuf Oreos.

```{r}
#| label: tbl-descStatsTable
...
Alternatively, we could express this in terms of a more conventional *t*-test by using a log transformation:

$$H_0:\log(\mu_D)-\log(\mu_R) = \log(2) \qquad \text{vs.} \qquad H_1:\log(\mu_D)-\log(\mu_R) \neq \log(2)$$

The log-transformation with a typically *t*-test and the ratio *t*-test are functionally equivalent.

## Assumptions

The *t*-test for the ratio of two group's values of the *sample arithmetic mean* makes two primary assumptions: Gaussian errors and independent observations. While the values of the *sample arithmetic standard deviation* are nearly identical, we will proceed with a version of the test that allows for heteroscedasticity.

@fig-qqPlots provides QQ plots for both Regular and Double Stuf Oreos when compared against a Gaussian distribution. For the regular Oreos (@fig-qqPlots-1), there are three points (`r round(3/36, 2)*100`%) located just outside of the envelope. For the Double Stuf Oreos, there is just one point (`r round(1/30, 2)*100`%). Pairing these plots with the values of *sample skewness* and *sample excess kurtosis* in @tbl-descStatsTable provide evidence that the assumption is relatively satisfied. That is to say, our data are not overtly inconsistent with the Gaussian assumption. The one point of hesitancy is the fact that the two groups have slight skewness in opposite directions. This could magnify the differences between them. However, it is worth noting that Double Stuf Oreos have a negative skewness suggesting that potential outliers are underfilled while regular Oreos have a postivie skewness suggesting just the opposite.  
...
#| fig-width: 2.75
#| fig-height: 2.75
#| fig-alt: "Left Q Q plot for regular Oreos shows little to be concerend about; same with the right Q Q plot for Double Stuf Oreos."
...
To assess the issue of independence, knowing the order in which measurements were taken is important. There is no explicit column given for the measurement order. I have spoken to Dr. Hatfield and he confirmed that the data appear in the file in measurement order. Thus, we can look at the index plot in @fig-indexPlot. I do not see any patterns in the measurements of filling mass that suggest a violation of independence. However, I will note that Discovery UK's How It's Made video documenting the creation of a similar cookie suggests that there could be a lack independence in the actual production process [@sandwich2019].

```{r }
#| label: fig-indexPlot
#| fig-cap: "Index Plot for Filling Mass"
#| fig-width: 5
#| fig-height: 2.5
#| fig-pos: "H"
#| fig-alt: "There do not appear to be any indications that the cookies suffer from problematic dependency captured by the measurement process."
...
With the assumptions tentatively satisfied, I'll proceed with the parametric shortcut for the the ratio test. I found that $t(`r round(ratioOut$parameter, digits = 2)`)\approx`r round(ratioOut$statistic, digits = 2)`$, with a *p*-value `r ifelse(test = ratioOut$p.value < 0.001, yes = "less than 0.001 (or 0.1%)", no = round(ratioOut$p.value, digits = 3))`. Given that this means that we would see a value at least as extreme as we have for our test statistic less than one-tenth of a percent of the time we repeat the study when the ratio of créme filling masses is 2 (i.e., `r round(ratioOut$p.value, 5)*100`%). Further, the *surprisal value* (*s*-value) for this study turns out to be `r round(-1*log(ratioOut$p.value, base = 2), 2)`; suggesting that this result is as surprising as getting `r round(-1*log(ratioOut$p.value, base = 2), 0)` heads in a row when flipping a fair coin. Taking both the *p*-value being less than our Unusualness Threshold and this level of suprise, we will take these data as evidence against the null hypothesis. The 97% confidence interval for the ratio based upon our data is $(`r round(ratioOut$conf.int[1], 2)`, `r round(ratioOut$conf.int[2], 2)`)$. Our specific sample of Oreos suggests that when used in conjunction with our selected 97% confidence interval estimation method based upon the ratio test, the ratio of filling performance might be between `r round(ratioOut$conf.int[1], 2)` and `r round(ratioOut$conf.int[2], 2)`.
...
# Appendix: Bootstrapping the Ratio of *SAM* Values
