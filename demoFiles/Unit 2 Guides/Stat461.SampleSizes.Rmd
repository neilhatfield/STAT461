...
The risk of making a Type II error is typically represented by \(\mathcal{E}_{II}\) or \(\beta\). Many times, researchers talk about __statistical power__, which is the complement of Type II risk, \(1-\mathcal{E}_{II}\). 
...
The graph shows us the interplay of effect size and total sample size. The smaller the effect size we're hoping to detect, the more measurement units we need. The larger th effect size, the fewer measurement units needed. Notice that there is not a linear relationship between these two aspects. 
...
One of the base packages of `R` is the `stats` package which is automatically loaded each time you start `R`. The function that you call is `power.anova.test`. To use this function to get _sample in each group_, 
