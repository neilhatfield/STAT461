...
In this context, we were using sex (dichotomous; Female, Not Female) as a block so that we can attempt to get a "cleaner"/better read on the impact of the estimation method (three levels; visual, tool, tool plus training). We can get the data from the URL that I emailed out:
...
barleyData$Treatment <- as.factor(barleyData$Treatment)
names(barleyData)[3] <- "Varietal"
names(barleyData)[4] <- "Order"
...
Keep in mind that all we are doing is adding new screens/terms into our model. Thus, fitting our new models is essentially just like in Unit 3. The only difference is that we place more terms after the `~` in our formula statements.
...
```{r dropObs, echo=TRUE, fig.cap="Updated QQ Plot for Estimation Errors Study", fig.width=4}
...
```{r estimationVar, fig.cap="Tukey-Anscombe Plots for Estimation Errors Study", fig.subcap=c("Full Data Set", "Dropping Extreme Potential Outlier"), fig.ncol=2, out.width="50%",}
# Demo code for making Tukey-Anscombe plot
...
names(pEst) <- c(
  "Grand Mean",
  levels(barleyData$Field),
  levels(barleyData$Varietal)
  )
...
Again, we interpret these are rates: bushels per acre per test plot or more simply, yield per plot.
