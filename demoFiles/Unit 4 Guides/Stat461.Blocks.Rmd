...
estimationData <- read.table(
  file = "https://raw.github.com/neilhatfield/STAT461/master/dataFiles/estimationErrors_Fall2022.csv",
  header = TRUE,
  sep = ","
)
...
estimationData$Method <- factor(
  ...
  levels = c("Visual", "Tool Only", "Tool and Training")
)
...
modelLabels <- c("1 Estimate Height 1", "2 Sex 1", "3 Method 2", "16 (Participants) 12")
modelMatrix <- matrix(
...
```{r gaussian, fig.cap="QQ Plots", fig.subcap=c("Farming Barley Study", "Estimation Errors Study"), fig.ncol=2, out.width="50%", echo=TRUE}
# Chunk options for side-by-side
### fig.subcap=c("Farming Barley Study", "Estimation Errors Study"), fig.ncol=2,
### out.width="50%"
...
For the Estimation Errors study our residuals appear to follow a Gaussian distribution.
...
For the Estimation Errors study, we can examine Figure \ref{fig:EstInteraction}.

```{r EstInteraction, fig.cap="Interaction Plot for Estimation Errors Study", fig.height=3, fig.width=5, echo=TRUE, fig.pos="H"}
# Demo code for an interaction plot
...
We can see in Figure \ref{fig:EstInteraction} that we have similar patterns of behavior as we look across levels of both our block and our factor.
...
### Your Turn--Explore the Estimation Error Data
...
For the Results portion of this guide/tutorial, I'm going to focus on the Farming Barley study. I'll leave the Estimation Errors study to you as an exercise.
