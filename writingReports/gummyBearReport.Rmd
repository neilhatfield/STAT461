...
-   Is there a statistically significant difference between a zero and non-zero launch angle (i.e., flat vs. low or high)?
...
We will break these into two testing families. The first will focus on the conditional comparisons of launch angle. The second will focus on the contrasts.
...
as.data.frame(
  ...
  knitr::kable(
    digits = 3,
    col.names = c("Comparison", "Launch Position", "Cohen's d", "Probability of Superiority"),
    align = "lccc",
...
Our second set of post hoc questions form a set of linear contrasts. Specifically, we want to explore whether the flat launch angle is different from the other two angles, regardless of launch position. Further, we want to compare the two pairs of treatment extremes: flat-front vs. high-back and flat-back vs. high-front. We will mention that this set of three contrasts are not orthogonal each other; however, we will still proceed.

```{r constrastCheck}
#| include = FALSE

# Check the orthogonality of the contrasts ----
c1 <- c(1/2, -1/4, -1/4, 1/2, -1/4, -1/4)
c2 <- c(0, 0, -1, 1, 0, 0)
c3 <- c(1, 0, 0, 0, 0, -1)
n <- 40

c1Vc2 <- sum(c1*c2)/n # 3/160 or 0.01875
c1Vc3 <- sum(c1*c3)/n # 3/160 or 0.01875
c2Vc3 <- sum(c2*c3)/n # 0
...
conOut <- emmeans::contrast(
  ...
  method = list(
    "Flat vs. Non-flat" = c(1/2, -1/4, -1/4, 1/2, -1/4, -1/4),
...
Table \ref{tab:postHocContrasts} presents the results of these three contrasts, with Benjamini-Hochberg adjusted *p*-values. We will decide to reject the null hypothesis for the first and third contrasts. We find that there is a significant difference in how far the gummy bear flies when comparing a flat launch angle (i.e., zero degrees) and either non-zero launch angle. Given our prior post hoc analysis, we note that this result might be entirely driven by the low-back treatment. There is some moderate practical significance here with the probability of the flat angle group giving the superior distance approximately 35% of the time.

While the flat-front and high-back pairing did not result in a statistical discovery, the flat-back and high-front did. There is a smaller practical difference in this later group as evidenced by the 41% probability of superiority and small magnitude of Cohen's *d*.
