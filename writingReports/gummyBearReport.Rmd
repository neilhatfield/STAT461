...
Each group had five gummy bears with which to practice. The distance that a gummy bear flew was the straight-line distance from the front edge of the meter stick (0 inch mark) to the point where the gummy bear came to rest; this distance was measured in centimeters. Each group had 40 gummy bears for their actual data collection; each of which they used only once. (The gummy bears were selected via cluster sampling from a randomly selected bag of gummy bears.) Each group recorded their own data and submitted their data files to Dr. Hatfield. Dr. Hatfield then merged the data files into one file [@hatfield2023]. Data are freely available on Dr. Hatfield's [GitHub repository](https://raw.github.com/neilhatfield/STAT461/master/dataFiles/gummyBears_Spring2023.xlsx) for STAT 461.

We will note here that we've used individual gummy bears as measurement units while the twelve groups as experimental units. Dr. Hatfield chose this set up to minimize the risk that each team might introduce confounding by having to switch their catapult setups for each individual gummy bear.

## Analytical Methods

To analyze our data and answer our research questions we will use R (version 4.2.3) and make use of ANOVA methods, in particular a full factorial (two-way) ANOVA model.

### Appropriateness of ANOVA

Given that our response of interest is how far a gummy bear traveled when flung by a spoonapult (in centimeters), we have a continuous response. Additionally, we have two factors of interest: the launch position and launch angle. Launch position is categorical with two levels (front or back). While launch angle could be quantified, we used stacks of books to induce angle and thus, we will treat this factor categorically as well with three levels (flat, low, or high). Both factors are fixed effects with these levels pre-specified by Dr. Hatfield.
...
Table \ref{tab:descpStats} shows the values for various statistics broken out by the six treatments. We can see that we have a balanced design (via the "n" column), which will provide us with benefits in terms of assumptions and *sums of squares* partitioning later on. There are a few aspects that jump out at us. First, the values for the *Sample Minimum* vary by several orders of magnitude; from 0 cm to 140 cm. Looking through the rest of the values comprising Tukey's Five-Number Summary for each treatment, the Flat-Back and Flat-Front conditions stand out as signs of potential issues.

If we wanted to explore the amount of variation within the treatment groups, we can turn towards two statistics: the *Median Absolute Deviation* (*MAD*) and the *Sample Arithmetic Standard Deviation* (*SASD*). The value of the *MAD* is the distance that half of all of the observations are from the value of the *Sample Median* and is a fairly robust and resistant estimator of the population standard deviation ($\sigma$). The *SASD* provides a unit-adjusted measure of amount of distance between pairs of observations within a collection, acting as a slightly biased, non-resistant estimator of the population standard deviation. Looking across the values of the *MAD*, we see some comparable values but we also see that the Low-Back treatment's and the Flat-Back treatment's value are under half the High-Front treatment's value. However, when we look along the values of the *SASD*, the values are more consistent with each other.
...
```{r histogram}
#| fig.cap = "Histogram of Distances",
#| fig.pos = "H",
#| fig.height = 2.75

# Establish Freedman-Diaconis Binwidth Rule ----
...
In the course of assessing assumptions for the parametric shortcut, we ran into some concerns. In particular,...
...
ggplot(
  data = data.frame(
    residuals = residuals(gummyModelFull), #
    fitted = fitted.values(gummyModelFull) #
  ),
  mapping = aes(x = fitted, y = residuals)
) +
  geom_point(size = 2) +
...
ggplot(
  ...
  geom_line(
    data = gummyData2,
    mapping = aes(y = sam, x = Measurement.Order),
    linetype = 2,
    color = "red"
  ) +
  theme_bw() +
