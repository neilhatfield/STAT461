...
There are limits to what permutation simulations can do. Their primary usage is for testing a nil hypothesis by helping us to estimate a *p*-value. This method is __great__ for this purpose. However, this approach is terrible for trying to generate any measure of effect size. This is primarily due to the fact that permutation distributions do not allow us to get dependable estimates of the sampling variation for any test statistic. Due to this, we should not use a permutation simulation to attempt to build a confidence interval for our test statistic. If we want such a confidence interval, we should consider the next simulation method.
