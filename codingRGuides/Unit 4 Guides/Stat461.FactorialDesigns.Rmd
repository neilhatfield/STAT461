...
catapultData$position <- as.factor(catapultData$position) # Order doesn't matter
...
For the Battery Manufacturing data I made use of the `recode_factor` function from the `dplyr` package. This allowed me not only transform/clean the reported values but also simultaneously tell R to treat the attribute as a factor.
...
Figure \ref{fig:gummyBearsIO} shows the index plots for the twelve catapult teams. I am going to leave to you the challenge of getting `R` to place the facets in the order Group 1, Group 2, Group 3, etc. instead of what currently appears.

When I look at Figure \ref{fig:gummyBearsIO}, I don't see twelve perfect carbon copies. This supports the idea that the twelve experimental units are independent. The plots I'm most concerned about are from Groups 4 and 8. For both of these groups there appears to be a decreasing trend in how far the gummy bears flew as they carried out their trials. 
