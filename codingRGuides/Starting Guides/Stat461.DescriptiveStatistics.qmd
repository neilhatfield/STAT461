...
The resulting table now fits on the page and has a better reading order. However, we can still improve the table by adding a caption.
...
summaryOreo %>%
  ...
  kable(
    caption = "Summary Statistics for Oreo Filling Masses", # <1>
    col.names = c("n", "Min", "Q1", "Median", "Q3", "Max", "MAD",
                  "SAM", "SASD", "Sample Skew", "Sample Ex. Kurtosis")
  )

```
1. In R Scripts and R Markdown documents, you'll use the `caption` argument of `kable`. For Quarto documents, you may use either this argument OR the `tbl-cap` code chunk option (but not both).

Be sure to place your caption inside a set of qoutation marks. 

Adding a caption to the table has an additional benefit in that we can now reference the table by name (e.g., @tbl-kableEx2b) in Markdown-enabled documents. For R Markdown, the enclosing code chunk should needs a unique name and only have this one output. For Quarto documents, the code chunk needs a unique label starting with `tbl-` and only have this one output. 
