...
## using the View function inside of a R Markdown file.

```

The `str` function I run in my console and will tell me the structure of the object whose name I pass as the input.
...
Because I'm reloading the Oreo data from the source file, I will need to tell my current session of R that Type should be a factor. In the penguins data, I see that species, island, and sex are already marked as factors. You'll also notice in the preview of values that there are a few `NA`s. `NA` denotes a missing value. We will see what happens in R when you go to use data with missing values.
...
The five quartiles for the filling mass data are: `r quantile(oreoData$Filling.Mass, na.rm = TRUE)` (found by using the command `quantile(oreoData$Filling.Mass, na.rm = TRUE)`). 
...
# Displaying Values in Professional Ways (Display Tables)
