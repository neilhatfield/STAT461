...
I'm going to use the built-in penguin data as well as a version of the Oreo data related to HW #1.1. As a reminder, here's how you load these both into R.
...
## Oreo Data (HW #1.1) 
oreoData <- read.table( # <2>
  file = "https://raw.github.com/neilhatfield/STAT461/master/dataFiles/classDemoOreo.dat", 
  header = TRUE,
  sep = ","
) 

## R versions older than 4.5.1 # <3>
# install.packages("palmerpenguins")  
# penguins <- palmerpenguins::penguins 

```
1. The `data` command will load any of built-in data set.
2. Here's how you can use the base R function `read.table` to load data from an online source. Be sure to update the URL to match you data.
3. If you are using a version of R that is older than 4.5.1, then the penguin data will not be built-in. You'll need to first install and load the `{palmerpenguins}` data.
...
A great way to help you think about data visualizations is the PCIP System. This approach gets you to think through a cycle of Plan-Code-Improve phases before moving into a final Polish phase. You can learn more about this approach at [this webpage](https://neilhatfield.github.io/STAT184/PCIP_System.html).
...
Many people are familiar with bar charts; they are a classic type of visualization. We can use either absolute or relative frequencies. An important difference between a bar chart and a dot plot is that bar charts *aggregate* our data. That is, while a dot plot shows cases that are individual (single) objects or living beings, the bar chart will hide these types of cases in favor of group case (i.e., a case that represents a collections of objects/living beings). Generally, bar charts are used with attributes that are categorical in nature such as the species of penguins or the type of Oreo.
...
A statistician and a mathematician worked together to form what is known as the Freedman-Diaconis rule which can help you come up with a potentially useful bin width. You can define this in R with the following function.
...
Another data visualization that people often mention are pie charts. Pie charts take the notion of a bar chart and plot things in a circular space. While in a bar chart we read the height of the bar to get the frequency value, pie charts work off of angle measures. That is, the angle measure for each slice reflects the relative proportion of the whole data collection. This makes pie charts great at relativistic comparisons between categories but terrible for getting accurate amounts. A second drawback to pie charts deals with deciding which slice has a larger angle measure when they are close to each other.
...
@fig-waffle1 shows a waffle plot for the penguins species data. For this visualization, I've opted to go the proportion route (via the `make_proportional = TRUE` argument). This means that each square in my 10 X 10 grid represents one percent of the data collection. If we instead wanted each square to represent an individual penguin, I'd use `make_proportional = FALSE` as shown in @fig-waffle2.
...
```
1. We can alter how many rows appear in the waffle plot. I changed to `n_rows = 8` since 344 (the sample size of the penguin data) is a multiple of 8.
...
4. For a horizontal box plot, map the case attribute to `x` and tell `y` to have no (`NULL`) label. If you want a vertical box plot, just swap `x` and `y` in the code.
5. These commands will make a cleaner box plot by getting rid of elements that don't have any meaning. For a vertical box plot, change the `.y` to `.x` as you swap the other `x` and `y` references.
...
The graphics that we create using `{ggplot2}` are very customizable and can allow us to do many things. One thing that we can do is layer multiple geometries on top of one another. This can allow us to leverage the strengths of different geometries to make up for the weaknesses of others. Consider the following plot. 
