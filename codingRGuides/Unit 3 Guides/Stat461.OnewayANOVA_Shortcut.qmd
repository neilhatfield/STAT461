---
title: "One-Way ANOVA"
subtitle: "Parametric Shortcut Guide"
author: 
...
Explore the data from any of the three studies used in this guide, the data from the class's first study, and/or any other data you have access to. What do you find? What do you notice?
...
## Checking the Unit 2 Requirements
...
## Checking the Unit 3 Requirement: Hasse Diagrams
...
parameters::model_parameters( # <1>
  ...
  kableExtra::kable_styling(
    font_size = 12,
    latex_options = c("HOLD_position") # <7>
  )
...
6. If you are knitting/render to PDF and keep running into a table that spills over the left/right edges of the page, add the `format` argument with the value `"latex"`.
7. The `latex_options` listed here are useful if you are using R Markdown/Quarto to create a PDF.
...
```{r}
#| label: getSvalue
# Code to convert p-value to s-value ----
sVal <- -1*log(anova(songModel)$`Pr(>F)`[[1]], base = 2)
```

#### Interpreting @tbl-songTable

We can see that a STAT461 undergraduate's year in school accounts for about 1.9 times as much variation as the residuals in @tbl-songTable. Drawing upon the balanced design to grant some robustness to the heteroscedasticity, we would anticipate seeing an *F* ratio at least as large as this ~20% of the time we would repeat the experiment if there truly was no impact of year in school on quiz score. Since our *p*-value is greater than our Unusualness Threshold (*UT*), we have an event that is "usual" (or typical) under the null hypothesis model and we would fail to reject the null hypothesis. Further, we can note that the *s*-value for this study is `r round(sVal, 2)` bits of information against the null hypothesis; that is to say, getting this result is only a little bit more surprising then getting two heads in a row when flipping a fair coin but less surprising than getting three heads in a row. We will decide to act as if a student's year in school does not impact how well they do on a song knowledge quiz.

:::{.callout-tip}
### *S*-Values
The *s*-value (a.k.a. Shannon information measure, binary surprise index, and surprisal) is transformation of *p*-values to assist with interpretations. Here, we imaging flipping (or tossing) a fair coin *s* times and getting the same result for every flip (i.e., all heads or all tails). The larger the value of *s*, the more surprising the result is and the more evidence we have against the idea that the coin is fair (i.e., our null hypothesis).

To convert a *p*-value into an *s*-value, we use the following formula $$s = -\log_2(p)$$
We can implement this formula in R with the command `sValue <- -1*log(pValue, base = 2)` where `pValue` is our *p*-value obtained from our test (you'll need to either provide this value or extract it from your model/summary object). We can then call `sValue` for use in our document.

Whole number values of *s* (e.g., 1, 2, 3, etc.) are interpreted as the number of successive coin flips all resulting in the same outcome. Non-whole number values of *s* aren't directly interpretable in this fashion, but you can use the whole numbers of either side as part of your interpretation, as was shown in the example. 
:::
...
```{r}
#| label: resinTable2
#| tbl-cap: "Modern ANOVA Table for Resin Lifetimes Study with Rounded P-Values"
#| html-table-processing: none
#| echo: true
#| eval: true
# Demo Code for Professional, Modern ANOVA Tables ----
...
```{=html}
<a href="https://neilhatfield.github.io/STAT461/codingRGuides/Unit%203%20Guides/Stat461.OnewayANOVA_Shortcut.html">One-Way ANOVA: Parametric Shortcut Guide</a> © 2026 by <a href="https://neilhatfield.github.io/">Neil J. Hatfield</a> is licensed under <a href="https://creativecommons.org/licenses/by-nc-sa/4.0/">CC BY-NC-SA 4.0</a><img src="https://mirrors.creativecommons.org/presskit/icons/cc.svg" alt="" style="max-width: 1em;max-height:1em;margin-left: .2em;"><img src="https://mirrors.creativecommons.org/presskit/icons/by.svg" alt="" style="max-width: 1em;max-height:1em;margin-left: .2em;"><img src="https://mirrors.creativecommons.org/presskit/icons/nc.svg" alt="" style="max-width: 1em;max-height:1em;margin-left: .2em;"><img src="https://mirrors.creativecommons.org/presskit/icons/sa.svg" alt="" style="max-width: 1em;max-height:1em;margin-left: .2em;">
```
