...
packages <- c("tidyverse", "hasseDiagram", "knitr", 
              "kableExtra", "car", "psych", "parameters", "emmeans")
...
Once you have set the `R` options, now comes reading in data. For this tutorial, we're going to work with three different data sets: the Fall 2023 Song Knowledge data, data from a honey study, and data from Example 3.2 Resin Lifetimes from the Oehlert textbook. 
...
parameters::model_parameters(
  ...
  caption = "Modern ANOVA Table for Fall '23 Song Knowledge Study",
  booktabs = TRUE,
  align = c("l", rep("c", 8))
  ) %>%
...
```{r confIntEst1, echo=TRUE}
# Produce Point and Interval Estimates for Marginal Means ----
resinMeans <- emmeans(
  object = resinModel, # Pass your ANOVA object
  specs = ~ temp, # Specify what element you want (i.e, your factor)
  level = 0.9, # This should be 1 - Type I Error 
  adjust = "bonferroni" # what adjustment method should be used
) 

resinMeans <- as.data.frame(resinMeans)

resinMeans %>%
  kable(
    digits = 4,
    caption = "Marginal Mean Estimates from the Resin Lifetime Study",
    col.names = c("Stress Temp. (ºC)", "Marginal Mean", "SE", "df", "Lower Bound", "Upper Bound"),
    booktabs = TRUE,
    align = "c"
  ) %>%
  kableExtra::kable_styling(
